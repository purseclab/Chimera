...
public class TopologyOperationGuidance extends FuzzIntentScenarioGuidance {
...
//    FuzzIntentGuidance intentGuidance;
    ...
    public FuzzScenario getRandomScenario(FuzzScenario scenario)
            throws IOException, JsonSyntaxException, SkipFuzzException {
        FuzzScenario newScenario = FuzzScenario.copy(scenario);
        ...
        while (true) {
            try {
                ...
                for (FuzzAction configAction : seedScenario.getConfigActions()) {
                    List<FuzzAction> newActionList = FuzzAction.fuzz(configAction);
                    newScenario.addConfigAction(newActionList);
                    if (newActionList.size() > 0 && !newActionList.get(0).equals(configAction))
                        isEqual = false;
                }
