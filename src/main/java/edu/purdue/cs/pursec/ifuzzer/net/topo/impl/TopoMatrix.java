...
public class TopoMatrix {
    ...
    String [][][] matrix;
    private final String matrixSelfId = "self";
    ...
    private int getPortNo(String portStr) {
        if (ConfigConstants.CONFIG_P4_PIPELINE.isEmpty()) {
            ...
            if (m.find()) {
                ...
                String parseStr = foundStr.substring(1, foundStr.length() - 1);
                return Integer.parseInt(parseStr);
            }
        }

        throw new NumberFormatException("wrong port no: " + portStr);
    }
    ...
    public void initialize(TopoGraph topoGraph) {
        ...
        for (TopoNode vertex : vertexSet) {
            if (vertex instanceof TopoHost) {
                ...
                matrix[0][j][j] = matrixSelfId;
                matrix[1][j][j] = matrixSelfId;
                j --;
        ...
        for (TopoEdge edge : activeEdges) {
            ...
            String srcPort = edge.getSrcPort();
            String dstPort = edge.getDstPort();

            if (edge.getSrcPort() != null) {
                srcPort = String.valueOf(getPortNo(edge.getSrcPort()));
                int nextSrcPort = getPortNo(edge.getSrcPort()) + 1;
            ...
            if (edge.getDstPort() != null) {
                dstPort = String.valueOf(getPortNo(edge.getDstPort()));
                int nextDstPort = getPortNo(edge.getDstPort()) + 1;
            ...
            matrix[0][srcIdx][dstIdx] = srcPort;
            matrix[1][srcIdx][dstIdx] = dstPort;
        }
    ...
    public JsonObject getRandomPointToPointIntent(JsonObject targetJson) {
        ...
        String srcPoint = matrix[0][srcIdx[0]][srcIdx[1]];
        String dstPoint = matrix[1][dstIdx[0]][dstIdx[1]];
        if (ConfigConstants.CONFIG_DISABLE_SAME_POINTS_OF_P2P_INTENT) {
            ...
            dstPoint = matrix[1][dstIdx[0]][dstIdx[1]];

        } else if (srcIdx[0] == dstIdx[1]) {
            // Same switch
            List<String> validPoints = new ArrayList<>();
            for (int i = 0; i < vertexId.length; i++) {
                String vertexPoint = matrix[0][srcIdx[0]][i];
                if (vertexPoint != null && !vertexPoint.equals(matrixSelfId))
                    validPoints.add(vertexPoint);
            }
            srcPoint = validPoints.get(rand.nextInt(validPoints.size()));
            dstPoint = validPoints.get(rand.nextInt(validPoints.size()));
        }

        targetJson.add("ingressPoint",
                createPointJson(vertexId[srcIdx[0]], srcPoint));
        targetJson.add("egressPoint",
                createPointJson(vertexId[dstIdx[1]], dstPoint));
