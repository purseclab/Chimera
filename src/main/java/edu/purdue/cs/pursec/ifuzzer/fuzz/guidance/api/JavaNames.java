...
//    }
    ...
    public static String getKeyFromMethod(String methodSignature, Set<String> classNames) {
        ...
        // remove '<' [className] ':'
        String methodClassName = firstLine.substring(1, firstLine.length() - 1).replace('.', '/');

        if (!classNames.contains(methodClassName))
            return null;

        String ret = methodClassName + ": ";
