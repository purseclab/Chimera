...
if __name__ == '__main__':
    ...
    parser.add_option("-D", "--device", dest="device_cls",
            help="specify the type of device",
            metavar="DEV", default="OVSSwitch")
    parser.add_option("-P", "--protocol", dest="device_proto",
            help="specify the protocol of device",
            metavar="PROTOCOL", default='OpenFlow13')
    ...
    parser.add_option("--ct", dest="ct",
            ...
            metavar="IP")
    parser.add_option("--switch-ip", dest="switchIpAddr",
            help="specify remote switch IP address",
            metavar="IP", default="localhost")
    parser.add_option("--switch-port", dest="switchGrpcPort",
            help="specify remote switch grpc port",
            metavar="INTEGER", type="int", default=50000)
    parser.add_option("--switch-log-level", dest="switchLogLevel",
            help="specify switch log level",
            metavar="LOG_LEVEL", default="warn")
    parser.add_option("--chassis-config-file", dest="chassisConfigFile",
            help="specify file path of chassis config of remote switch",
            metavar="PATH")


    (options, args) = parser.parse_args()
    if options.device_cls.lower() == 'bmv2' and \
            options.device_proto == 'OpenFlow13':
        options.device_proto = "org.onosproject.pipelines.basic"
